...
export const joinQueue = (interaction: ButtonInteraction<CacheType>): void => {
	...
	const playerQuery = Player.findOne<IPlayer>({
		discordId: interaction.user.id,
	});
	const queueQuery = Queue.find<IQueue>().and([
		{ messageId: interaction.message.id },
		{ discordId: interaction.user.id },
	]);
	const queuePlayers = Queue.find<IQueue>({
		messageId: interaction.message.id,
	});

	Promise.all([playerQuery, queueQuery, queuePlayers])
		.then(async (queryResults: [IPlayer | null, IQueue[], IQueue[]]) => {
			if (!queryResults[0]) {
