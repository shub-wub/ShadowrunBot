...
import { CacheType, ChannelType, CommandInteraction, GuildEmoji, MappedGuildChannelTypes, Role, TextChannel } from "discord.js";
import Guild from "../../schemas/guild";
import { MongooseError } from "mongoose";
import { IGuild } from "../../types";

export const srinitialize = async (interaction: CommandInteraction<CacheType>): Promise<void> => {
    const existingGuildRecord = await Guild.findOne<IGuild>({ guildId: interaction.guildId });
    ...
    const bronzeRoleCreate = interaction.guild?.roles.create({ name: 'Bronze', color: "DarkOrange", reason: 'ShadowrunBot Ranked'});
    const silverRoleCreate = interaction.guild?.roles.create({ name: 'Silver', color: "Greyple", reason: 'ShadowrunBot Ranked'});
    const goldRoleCreate = interaction.guild?.roles.create({ name: 'Gold', color: "Gold", reason: 'ShadowrunBot Ranked'});
    const platinumRoleCreate = interaction.guild?.roles.create({ name: 'Platinum', color: "DarkGreen", reason: 'ShadowrunBot Ranked'});
    const diamondRoleCreate = interaction.guild?.roles.create({ name: 'Diamond', color: "Aqua", reason: 'ShadowrunBot Ranked'});
    
    const bronzeEmojiCreate = interaction.guild?.emojis.create({attachment: './assets/bronze.webp', name: 'bronze', reason: 'ShadowrunBot Ranked'});
    const silverEmojiCreate = interaction.guild?.emojis.create({attachment: './assets/silver.webp', name: 'silver', reason: 'ShadowrunBot Ranked'});
    const goldEmojiCreate = interaction.guild?.emojis.create({attachment: './assets/gold.webp', name: 'gold', reason: 'ShadowrunBot Ranked'});
    const platinumEmojiCreate = interaction.guild?.emojis.create({attachment: './assets/platinum.webp', name: 'platinum', reason: 'ShadowrunBot Ranked'});
    const diamondEmojiCreate = interaction.guild?.emojis.create({attachment: './assets/diamond.webp', name: 'diamond', reason: 'ShadowrunBot Ranked'});
